...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    ...
    entries.forEach(([key, value]) => {
      const hasKeyProperty = Object.prototype.hasOwnProperty.call(result, key);
      if (hasKeyProperty) {
        result[key] += value;
      } else {
        result[key] = value;
...
function makeWord(lettersObject) {
  ...
  keys.forEach((letter) => {
    const poistionsArr = lettersObject[letter];
    poistionsArr.forEach((position) => {
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (money === 100) {
      if (wallet['25'] === 0 || (wallet['25'] < 3 && wallet['50'] === 0)) {
        return false;
      }
      if (wallet['50']) {
        wallet['50'] -= 1;
